...
class WavfileTest(TestCase):
    def _test_single_file(self, filepath, f):
        _, size, comp, noc, rate, __, ba, bits, bytes, dtype = wavfile.read_wav_info(filepath)
        f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(filepath, size, comp, noc, rate, ba, bits, bytes, dtype))
    ...
    def test_get_wav_info(self):
        ...
        configs = {}
        with open('/tmp/wavefile.tsv', 'w') as f:
            f.write('wavefile\tsize\tcomp\tnoc\trate\tba\tbits\tbytes\tdtype\n')
            for idx, af_file_path in enumerate(files_to_test):
                _, size, comp, noc, rate, __, ba, bits, bytes, dtype = wavfile.read_wav_info(af_file_path)

                key = (size, comp, noc, rate, ba, bits, bytes, dtype)
                if key not in configs:
                    configs[key] = af_file_path
                    f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(af_file_path, size, comp, noc, rate, ba, bits, bytes,
                                                                          dtype))


                # print('Testing {}/{}: {}'.format(idx + 1, n_files_to_test, af_file_path))
                # self._test_single_file(af_file_path, f)
