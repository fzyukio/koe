...
import {
  ...
  SLickValidator,
  SlickEditors,
  RowMoveableFormatter,
} from "./slick-grid-addons";
    ...
    if (char1 === "=") {
      ...
      for (let i = 0; i < rangeOperators.length; i++) {
        ...
        if (rangeStart > -1) {
          ...
          let upper = filterContent.substring(
            rangeStart + rangeOperator.length
          );
...
const insertOrReplaceColumn = function (columns, newcol) {
  ...
  if (index == -1) {
    ...
    for (let attr in oldColumn) {
      if (
        Object.prototype.hasOwnProperty.call(oldColumn, attr) &&
        !Object.prototype.hasOwnProperty.call(newcol, attr)
      ) {
 ...
 */
export const renderSlickGrid = function (
  selector,
  grid,
  rows,
  columns,
  ...
  if (multiSelect) {
    selectorPlugin = new Slick.CheckboxSelectColumn({
      cssClass: "checkboxsel",
    });
  }
  if (radioSelect) {
  ...
  grid.onBeforeCellEditorDestroy.subscribe(function (e, args_) {
    ...
    let currentElement = $(
      args_.grid.getCellNode(currentCell.row, currentCell.cell)
    );
    currentElement.parent().find("[role=tooltip]").remove();
    currentElement
      .attr("data-placement", undefined)
  ...
  grid.onValidationError.subscribe(function (e, args_) {
    ...
    element
      .attr("data-placement", toolTipPlace)
      .attr("data-original-title", "Input error")
