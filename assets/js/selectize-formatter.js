...
export const constructSelectizeOptionsForConstants = function (
  selectOptions,
  defaultValue
) {
  let options = [];
...
export const constructSelectizeOptionsForLabellings = function (
  field,
  defaultValue
) {
  ...
  for (let option in selectableOptions) {
    if (
      option &&
      Object.prototype.hasOwnProperty.call(selectableOptions, option)
    ) {
...
export const SelectizeEditor = function (args) {
  ...
  this.init = function () {
    ...
    if (args.column._formatter == "Select") {
      selectizeOptions = constructSelectizeOptionsForConstants(
        args.column.options,
        defaultValue
      );
    } else {
      selectizeOptions = constructSelectizeOptionsForLabellings(
        args.column.field,
        defaultValue
      );
    }
