...
const saveSongsToDb = function () {
  ...
  let uploadPromise = itemsToUpdate.reduce(function (promiseChain, item, i) {
    ...
    let endSample = Math.min(
      Math.ceil((endMs * nSamples) / durationMs),
      nSamples
    );
    ...
    formData.append(
      "track-id",
      trackInfoForm.find("#id_track_id").attr("value")
    );
      ...
      onSuccess(res) {
        ...
        if (trackInfoForm.find("#id_track_id").attr("valid") === "true") {
          ...
          options.push({
            value: "$track-name",
            text: "Track name",
            explain: `Name of the track, in this case it's "${name}"`,
            isVar: true,
          });
          ...
          extraArgs["track_id"] = trackInfoForm
            .find("#id_track_id")
            .attr("value");
...
function handleInputAdded(value, data) {
  ...
  if (sanitised !== value) {
    let message =
      "Your pattern contains special characters which I just removed. Only numbers, alphabets, underscores and dashes are allowed.";
...
function initSelectize(options) {
  ...
  namePatternInput = initSelectizeSimple(
    $songNamePatternInput,
    options,
    defaultSelectizeArgs
  );
...
export const run = function (commonElements) {
  ...
  loadSongPromise().then(function ({
    ...
    audioData.durationRatio =
      realLength / realFs / (audioData.browserLength / audioData.browserFs);
