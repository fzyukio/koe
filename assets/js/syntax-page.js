  ...
  for (let i = 0; i < nClasses; i++) {
    ...
    for (let j = 0; j < nNearest; j++) {
      ...
      if (existingPairs.indexOf(`${class1Label}-${neighbourName}`) == -1) {
        ...
        rows.push({
          id: pairId,
          "class-1-name": class1Label,
          "class-2-name": neighbourName,
          "syntax-distance": stxDistance,
          "acoustic-distance": acsDistance,
          "weighted-distance": weightedDistance,
          "class-1-count": class1Count,
          "class-2-count": class2Count,
        });
...
const downloadTensorData = function () {
  ...
  return Promise.all([downloadMeta, downloadBytes, downloadSyntaxData]).then(
    function (values) {
      let meta = values[0];
      let bytes = values[1];
...
function recordMergeInfo(newClassName) {
  ...
  postRequest({
    ...
    data: {
      ...
      "class1-ids": JSON.stringify(class1RowIds),
      "class2-ids": JSON.stringify(class2RowIds),
      "class1n2-name": newClassName,
    },
...
function mergeClasses(newClassName) {
  ...
  let mergedClassAdjRow = nj
    .array(adjMat[class1Ind])
    .add(nj.array(adjMat[class2Ind]))
    .tolist();
  ...
  let mergedClassAdjRowCol = nj
    .array(adjMatRowMerged[class1Ind])
    .add(nj.array(adjMatRowMerged[class2Ind]))
    .tolist();
