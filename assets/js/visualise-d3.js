...
export const Visualise = function () {
        ...
        .on("brushend", function () {
            ...
            if (viz.spectBrush.empty()) {
                ...
                if (sylIdx === undefined) {
                    ...
                    let newSyllable = {id: newId, x0: start, x1: end, y0: 0, y1: 1};
                    syllables[newId] = newSyllable;

                    viz.eventNotifier.trigger("segment-changed", {type: "segment-created", target: newSyllable});
    ...
    this.displaySegs = function (syllables) {
        ...
        for (let sylIdx in syllables) {
            if (syllables.hasOwnProperty(sylIdx)) {
                let syl = syllables[sylIdx];
                let begin_ms = syl.start_time_ms;
                let end_ms = syl.end_time_ms;

                let begin_sec = begin_ms / 1000 / actualDuration * viz.imgWidth;
                let end_sec = end_ms / 1000 / actualDuration * viz.imgWidth;
        ...
        $(viz.spectrogramId + ' .syllable').each(function (idx, el) {
            ...
            if (!el.hasClass('mouse-behaviour-attached')) {
                el.on('click', function (event, a, b) {
                    // Left click
                    let self = this;
                    if (event.which === 1) {
                ...
                .on('mouseover', function () {
                    ...
                    let start_sec = syl.start_time_ms / 1000 / actualDuration * spectrogramDuration;
                    let end_sec = syl.end_time_ms / 1000 / actualDuration * spectrogramDuration;
                    ...
                    // viz.spectBrush.extent([start_sec, end_sec]);
                    viz.spectBrush.extent([start_sec, end_sec]);
                    ...
                    viz.eventNotifier.trigger("segment-mouse", {type: "segment-mouseover", target: this});
                })
                .on('mouseleave', function (event) {
                    ...
                    // External process might be interested in this event too
                    viz.eventNotifier.trigger("segment-mouse", {type: "segment-mouseleave", target: this});
                });
