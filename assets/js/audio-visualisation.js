...
export class Visualiser {
  ...
  constructor(vizContainerId) {
    ...
    this.spectWidth =
      this.$spectrogram.height() - this.margin.left - this.margin.right;
    this.spectHeight =
      this.$spectrogram.height() -
      this.margin.top -
  ...
  resetArgs(args) {
    ...
    if (changed.zoom) {
      ...
      self.downSampleFactor = Math.max(
        defaultDownSampleFactor,
        Math.floor(defaultDownSampleFactor / (self.zoom / 100))
      );
  ...
  displayAsPromises() {
    ...
    let end = Math.min(
      self.lastChunkIdx,
      self.currentImageIndex + self.numberOfImages
    );
    ...
    return sliced.reduce(function (promiseChain, chunk) {
      ...
      let subSig = self.sig.subarray(
        subSegs[0][0],
        subSegs[subSegs.length - 1][1]
      );
      let renderStatus = self.spectRenderingStatus[segBeg];
      ...
      let oscilXScale = d3
        .scaleLinear()
        .range([0, subImgWidth])
        .domain([0, subSig.length]);
      let oscilYScale = d3
        .scaleLinear()
        .domain([self.minY, self.maxY])
        .nice()
        .range([self.oscilloHeight, 0]);
      ...
      return promiseChain
        .then(function () {
          ...
          if (spect === null) {
            ...
            spect = transposeFlipUD(
              calcSpect(
                self.sig,
                subSegs,
                self.fft,
                self.fftComplexArray,
                self.window,
                self.windowed
              )
            );
        ...
        .then(function (dataURI) {
          if (dataURI) {
            ...
            osc
              .append("path")
              .attr("class", oscillogramLineClass)
  ...
  visualiseSpectrogram() {
    ...
    if (!self.imagesAreInitialised) {
      self.chunks.forEach(function (chunk) {
        ...
        self.spectrogramSpects
          .append("image")
          .attr("id", `spect-${segBeg}`)
          .attr("height", self.imgHeight)
          .attr("width", subImgWidth)
          .attr("x", segBeg)
          .style("transform", `scaleY(${self.spectHeight / self.imgHeight})`);
        self.oscillogramPaths
          .append("g")
          .attr("id", `oscil-${segBeg}`)
          .style("transform", `translateX(${segBeg}px)`);
  ...
  drawBrush() {
    ...
    self.spectBrush.extent([
      [range[0], 0],
      [range[1], self.spectHeight],
    ]);
    ...
    self.spectBrush
      .on("start", function () {
        ...
        if (d3.event.selection) {
          ...
          self.playbackIndicator
            .style("display", "unset")
            .attr("transform", `translate(${startPixel}, 0)`);
      ...
      .on("end", function () {
        ...
        if (d3.event.selection === null) {
          ...
          debug(
            "Current resizeable syllable index: " +
              window.appCache["resizeable-syl-id"]
          );
        } else {
    ...
    this.spectBrushEl = this.spectrogramSvg
      .append("g")
      .attr("class", "spect-brush");
    this.spectBrushEl.call(this.spectBrush);

    self.spectHandle = this.spectBrushEl
      .selectAll(".brush-handle")
      .data([
        { type: "w", spectHeight: self.spectHeight },
        { type: "e", spectHeight: self.spectHeight },
      ])
      .enter()
      .append("path")
  ...
  initCanvas() {
     ...
     */
    self.imgWidth =
      Math.floor((self.audioData.browserLength - self.nfft) / self.frameSize) +
      1;
    self.imgHeight = self.nfft / 2;

    self.spectrogramSvg = d3.select(self.spectrogramId);
    self.spectrogramSvg.selectAll("*").remove();
    self.spectrogramSvg.attr(
      "height",
      self.spectHeight + self.margin.top + self.margin.bottom
    );
    self.spectrogramSvg.attr(
      "width",
      self.imgWidth + self.margin.left + self.margin.right
    );
    ...
    self.spectXScale = d3
      .scaleLinear()
      .range([0, self.imgWidth])
      .domain(spectXExtent);
    self.spectYScale = d3
      .scaleLinear()
      .range([self.spectHeight, 0])
      .domain(spectYExtent);
    ...
    self.spectrogramAxis.attr(
      "transform",
      "translate(0," + self.spectHeight + ")"
    );
    self.spectrogramAxis.call(xAxis);

    /* Remove the first tick at time = 0 (ugly) */
    self.spectrogramAxis
      .selectAll(".tick")
      .filter((d) => d === 0)
      .remove();

    self.spectrogramSpects = self.spectrogramSvg
      .append("g")
      .classed("spects", true);
    self.oscillogramPaths = self.oscillogramSvg
      .append("g")
      .classed("oscils", true);

    self.spectrogramSvg
      .append("line")
      .attr("class", "playback-indicator")
      .attr("x1", 0)
      .attr("y1", 0)
      .attr("x2", 1)
      .attr("y2", self.spectHeight)
  ...
  startPlayback(begin = 0, end = "end", scroll = false) {
    ...
    playAudioArgs.onStartCallback = function (_playbackSpeed) {
      ...
      let durationAtSpeed =
        ((durationMs * 100) / playbackSpeed) * self.audioData.durationRatio;
  ...
  showBrush(sylIdx) {
    ...
    if (resizing && self.editMode) {
      debug(
        `Currently editing another segment, ignore. resizing = ${resizing}`
      );
      return;
    }
    ...
    self.spectBrush.move(
      self.spectrogramSvg.select(".spect-brush"),
      [syl.start, syl.end].map(self.spectXScale)
    );
  }
  ...
  startScrolling(startX, endX, duration) {
    ...
    let delayStart =
      visualisationWidth / 2 - (startX - self.visualisationEl.scrollLeft);
    ...
    self.scrollingTimer = setTimeout(function () {
      self.scrollingPromise = smoothScrollTo(
        self.visualisationEl,
        scrolltarget,
        scrollDuration
      );
    }, delayStartDuration);
  ...
  highlightSegments(e, args) {
    ...
    let spectrogramSegment = self.$spectrogram.find(
      `rect.syllable[syl-id="${songId}"]`
    );
  ...
  initController() {
    ...
    pauseSongBtn.click(function () {
      ...
      self.stopScrolling();
      let newStartPoint =
        parseInt(resumeSongBtn.attr("start-point")) + playedDuration;
