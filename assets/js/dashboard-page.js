...
const subscribeFlexibleEvents = function () {
  ...
  function onCollaboratorSelectionChange(e, args) {
    let grid_ = args.grid;
    let canRemoveCollaborator =
      removeCollaborator.hasAdminPrivileges &&
      grid_.getSelectedRows().length > 0;
...
const initApplyVersionBtn = function () {
  applyVersionBtn.click(function () {
    ...
    ce
      .dialogModalBody.html(`Importing history from will erase your current data.
...
const reloadSyllableGrid = function () {
  return syllableGrid
    .initMainGridHeader(syllableGridArgs, syllableExtraArgs)
    .then(function () {
      return syllableGrid.initMainGridContent(
        syllableGridArgs,
        syllableExtraArgs
      );
    });
...
export const run = function (commonElements) {
  ...
  let collectionPromise = collectionGrid
    .initMainGridHeader(collectionGridArgs, collectionExtraArgs)
    .then(function () {
      return collectionGrid.initMainGridContent(
        collectionGridArgs,
        collectionExtraArgs
      );
    });

  let dbGridPromise = databaseGrid
    .initMainGridHeader(databaseGridArgs, databaseExtraArgs)
    .then(function () {
      databaseGrid.on("row-added", function (e, args) {
        ...
        return Promise.all([
          dbAssignmentGrid.initMainGridHeader(
            dbAssignmentGridArgs,
            dbAssignmentExtraArgs
          ),
          ...
          .then(function () {
            return Promise.all([
              dbAssignmentGrid.initMainGridContent(
                dbAssignmentGridArgs,
                dbAssignmentExtraArgs
              ),
              versionGrid.initMainGridContent(
                versionGridArgs,
                versionExtraArgs
              ),
            ]);
          ...
          .then(function () {
            let hasNoCollaborators =
              dbAssignmentGrid.mainGrid.getData().getItems().length == 1;
      ...
      return databaseGrid.initMainGridContent(
        databaseGridArgs,
        databaseExtraArgs
      );
    });
...
function initCreateDatabaseButton() {
  ReactDOM.render(
    <NewDatabaseButton databaseGrid={databaseGrid} />,
    document.getElementById("create-database-button-wrapper")
  );
...
const initEnterInvitationCodeBtn = function () {
  ...
  function showDialog(error) {
    ...
    dialogModalOkBtn.one("click", function () {
      ...
      $.post(url, { code: invitationCode })
        .done(function (data) {
          data = JSON.parse(data);
          let row = data.message;
          dialogModal.one("hidden.bs.modal", function () {
            let existedItem = databaseGrid.mainGrid
              .getData()
              .getItemById(row.id);
            if (existedItem) {
...
function initAddCollaboratorBtn() {
  ...
  function showDialog(error) {
    ...
    dialogModalBody.html(
      "<label>Name or email address of your collaborator</label>"
    );
    dialogModalBody.append(inputText);
    if (error) {
      dialogModalBody.append(`<p>${error.message}</p>`);
...
function initDeleteCollectionsBtn() {
  deleteCollections.click(function () {
    ...
    ce.dialogModalBody.html(
      `Are you sure you want to delete the following collections?<br>${itemNames.join(
        "<br>"
      )}`
    );
...
function initRemoveCollaboratorBtn() {
  removeCollaborator.click(function () {
    ...
    ce.dialogModalTitle.html(
      `Confirm removing ${itemIds.length} collaborator(s)`
    );
    ce.dialogModalBody.html(
      `Are you sure you want to remove the following collaborator?<br>${itemNames.join(
        "<br>"
      )}`
    );
