...
const KEY_CONVERTERS = {
  ...
  DecimalPoint: (x) => parseFloat(parseFloat(x).toFixed(2)),
  _: (x) => x,
};
...
class SyllableCsvUploader extends CsvUploader {
  ...
  getMatchedAndChangedRows(
    ...
    return SyllableCsvUploader.getUnsegmentedSongs()
      .then(function (unsegmentedSongs) {
...
const initDeleteSegmentsBtn = function () {
  deleteSegmentsBtn.click(function () {
    ...
    ce
      .dialogModalBody.html(`Are you sure you want to delete these syllables and all data associated with it? 
...
const initCreateTemporaryDatabaseBtn = function () {
  ...
  function showDialog(error) {
    ...
    dialogModalBody.html(
      `<label>We gave it the temporary name of ${generatedName}. Do you want to give it a different name?</label>`
    );
  ...
  makeTemporaryDatabaseBtn.click(function (e) {
    ...
    postRequest({
      ...
      onFailure(message) {
        ...
        dialogModalBody.html(
          `<p>You've already created a Collection with name <strong>${message}</strong>. Please choose a unique name.</p>`
        );
