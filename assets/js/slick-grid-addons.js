...
export const editabilityAwareFormatter = function (
  row,
  cell,
  value,
  columnDef,
...
const FloatEditorRewritten = function (args) {
  ...
  this.init = function () {
    $input = $(
      '<INPUT type="number" inputmode="numeric" step="0.01" class=\'editor-text\' />'
    );
...
const SpeciesEditor = function (args) {
  ...
  this.isValueChanged = function () {
    ...
    return (
      value.genus != $genus.val().trim() ||
      value.species != $species.val().trim()
    );
  };
  this.validate = function () {
    ...
    if (!genus.match(regEx) || !species.match(regEx)) {
      return {
        valid: false,
        msg: "Genus and Species must be alphabetic and start with an uppercase letter",
      };
    }
