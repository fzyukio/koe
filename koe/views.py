...
from koe.forms import (
    ContactUsForm,
    FeatureExtrationForm,
    OrdinationExtractionForm,
...
class SegmentationView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        song_extra_attr_values_list = ExtraAttrValue.objects.filter(
            user=user, attr__klass=AudioFile.__name__, owner_id=audio_file.id
...
class SongPartitionView(FormView):
    ...
    def form_invalid(self, form):
        ...
        retval = dict(
            origin="SongPartitionView.form_invalid",
            success=False,
            warning=None,
            payload=rendered,
        )
    ...
    def form_valid(self, form):
        ...
        rendered = render_to_string("partials/track-info-form.html", context=context)
        retval = dict(
            origin="SongPartitionView.form_valid",
            success=True,
            warning=None,
            payload=rendered,
        )
...
def get_incomplete_tasks(target_class):
    ...
    subtasks = Task.objects.filter(
        parent__in=all_incomplete_dms.values_list("task", flat=True),
...
def extra_syllables_context(request, context):
    ...
    if database:
        ...
        if isinstance(database, Database):
            ...
            cur_sim_val = ExtraAttrValue.objects.filter(
                attr=settings.ATTRS.user.database_sim_attr,
                user=user,
                owner_id=database.id,
            ).first()
        else:
            similarities = similarities.filter(Q(dm__tmpdb=database) | Q(ord__dm__tmpdb=database))
            cur_sim_val = ExtraAttrValue.objects.filter(
                attr=settings.ATTRS.user.tmpdb_sim_attr, user=user, owner_id=database.id
            ).first()
