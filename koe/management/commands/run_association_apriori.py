...
class Command(BaseCommand):
    ...
    def handle(self, database_name, username, label_level, *args, **options):
        segs = Segment.objects.filter(audio_file__database__name=database_name).order_by(
        ...
        labels = ExtraAttrValue.objects.filter(
            attr=label_attr, owner_id__in=seg_ids, user__username=username
        ...
        for mined_object in mined_objects:
            ...
            print(
                "{0:>9d} {1:>0.7f} {2:>9s} {3:>9s} {4:>80s} ".format(
