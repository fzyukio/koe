...
from koe.models import Aggregation, Database, Feature, Segment
...
def extract_mfcc_multiparams(database_name, save_dir, ncep, fmin, fmax):
    ...
    vals = list(
        segments.order_by("audio_file", "start_time_ms").values_list(
            "audio_file__name", "tid", "start_time_ms", "end_time_ms"
        )
    )
    ...
    for feature in features:
        tid2fval = {}
        saved_file = "database={}-feature={}-fmin={}-fmax={}-ncep={}.pkl".format(
            database_name, feature.name, fmin, fmax, ncep
        )
        ...
        for aggregator in aggregators:
            tid2aval = {}
            agg_saved_file = "database={}-feature={}-aggregator={}-fmin={}-fmax={}-ncep={}.pkl".format(
                database_name, feature.name, aggregator.get_name(), fmin, fmax, ncep
            )
