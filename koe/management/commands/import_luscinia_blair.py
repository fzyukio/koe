    ...
    if not os.path.isfile(wav_file_path):
        ...
        try:
            ...
            with open(fname, 'wb') as f:
                pickle.dump(dict(data=data, song=song), f, pickle.HIGHEST_PROTOCOL)
            warning('Song #{} cannot be import. Raw data saved to {}'.format(song_id, fname))
            wav_exists = False
    else:
        fs, length = get_wav_info(wav_file_path)

    if wav_exists and not os.path.isfile(mp3_url):
...
def extract_spectrogram(database):
    ...
    :return:
    """
    values_list = Segment.objects.filter(segmentation__audio_file__database=database)[:25].values_list('id', 'segmentation__audio_file__id', 'segmentation__audio_file__name','start_time_ms', 'end_time_ms')
    audio_to_segs = {}
    ...
    for (song_name, song_id), seg_list in audio_to_segs.items():
        ...
        try:
            ...
            # global_min_spect_pixel = np.min(np.min(file_spect))
            # global_max_spect_pixel = np.max(np.max(file_spect))
            # global_spect_pixel_range = global_max_spect_pixel - global_min_spect_pixel
            # interval64 = global_spect_pixel_range / 63
...
def compress_data(database):
    ...
    segments_ids = Segment.objects.filter(segmentation__audio_file__database=database).values_list('id', flat=True)
    ...
    for a in audio_files:
        song_mp3_path = mp3_path(a)

        if os.path.isfile(song_mp3_path):
            tar.add(song_mp3_path)
        bar.next()
