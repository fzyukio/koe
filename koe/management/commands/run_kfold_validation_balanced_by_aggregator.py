    ...
    def handle(self, *args, **options):
        ...
        if csv_filename:
            with open(csv_filename, "w", encoding="utf-8") as f:
                if source == "pca":
                    f.write(
                        "Feature group\tAggregators\tNdims\tPCA explained\tPCA Dims\tLabel prediction mean\tstdev"
                        "\t{}\n".format("\t ".join(unique_labels))
                    )
                else:
                    f.write(
                        "Feature group\tAggregators\tNdims\tLabel prediction mean\tstdev\t{}\n".format(
                            "\t ".join(unique_labels)
                        )
                    )
        ...
        for ftgroup_name, feature_names in ftgroup_names.items():
            for agggroup_name, aggs in list(enabled_aggregators.items()) + [("all", None)]:
                ...
                bar = Bar(
                    "Features: {}. Aggregator: {}. Classifier: {} Data type: {}...".format(
                        ftgroup_name, agggroup_name, clsf_type, source
                    ),
                    max=ntrials,
                )
