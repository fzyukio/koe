...
def get_max_length(list_of_sids):
    """
    Find the length of the longest syllables by querying the database
    :param list_of_sids: list of syllable IDs (sid not tid)
    :return: length of the longest sylalble in millisecond
    """
    start_end_times = Segment.objects.filter(id__in=list_of_sids).values_list('start_time_ms', 'end_time_ms')
    durations = [end - start for start, end in start_end_times]
...
def train(variables, save_to):
    ...
    max_length = get_max_length(sids_for_training + sids_for_testing)
    ...
    def get_batch(this_batch_size=10, data_type='train'):
        ...
        for i, sid in enumerate(batch_ids):
            ...
            with open(spect_path, 'rb') as f:
                ...
                # Pad it to have max_length
                nfreqbins, duration = spect.shape
                pad_length = max_length - duration

                pad_value = np.zeros((nfreqbins, pad_length))
                padded_spect = np.concatenate((spect, pad_value), axis=1)
    ...
    factory.input_dim = variables['dims'] * max_length
    factory.output_dim = variables['dims'] * max_length
    factory.padded_length = max_length
